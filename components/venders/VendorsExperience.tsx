...
export default function VendorsBoxesExperience() {
  ...
  const toPrev = useCallback(() => {
    ...
    setActiveIndex((prev) => (prev === 0 ? vendors.length - 1 : prev - 1));
  }, [isAnimating]);

  const toNext = useCallback(() => {
    if (isAnimating) return;
    setActiveIndex((prev) => (prev === vendors.length - 1 ? 0 : prev + 1));
  }, [isAnimating]);
    ...
    <div className="relative w-full h-screen bg-gradient-to-b from-[#FAFAFA] to-[#F0F0F0] overflow-hidden">
      ...
      <div className="absolute top-12 left-0 right-0 z-20 text-center px-6 pointer-events-none">
        <h1 className="text-4xl md:text-5xl font-bold text-gray-900 tracking-tight">
          {vp?.expTitle1?.[language] || "Our Trusted"}{" "}
          <span className="text-[#00c050]">
            {vp?.expTitle2?.[language] || "Vendors"}
          </span>
        </h1>
      </div>

      {/* Tilt Stack Carousel — sits above the 3D scene */}
      <div
      ...
      >
            ...
            {vendors.map((vendor, i) => {
              ...
              return (
                <div
                  ...
                  style={{
                    perspective: 1000,
                    zIndex: vendors.length - Math.abs(activeIndex - i),
                  }}
                >
                  <motion.div
                    onClick={() => toSlide(i)}
                  ...
                  >
                    <img
                      src={vendor.logo}
                      alt={vendor.name}
                      className="max-h-[65%] max-w-[75%] object-contain drop-shadow-sm"
                    />
                  </motion.div>
                </div>
              );
        ...
        {/* Controls */}
        <div className="flex items-center gap-4 px-3 py-2 rounded-full bg-white/70 backdrop-blur-sm border border-gray-200/90 shadow-sm">
          <button
            onClick={toPrev}
            className="p-2 rounded-full text-gray-600 hover:text-[#00c050] transition-colors"
          >
            ...
            {vendors.map((_, i) => (
              <button
                ...
                className={`rounded-full h-1.5 transition-all duration-300 ${
                  activeIndex === i ? "w-6 bg-[#00c050]" : "w-1.5 bg-gray-300"
                }`}
